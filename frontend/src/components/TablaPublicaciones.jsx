...
import { DialogActions, DialogTitle, DialogContent, FormControl, FormLabel, FormGroup, FormControlLabel, Switch } from "@material-ui/core";
...
export const TablaPublicaciones = () => {
  ...
  const addPosts = post => {
    ...
    .then(() => {
      // Recargar los posts después de añadir uno nuevo
      fetch('http://localhost:8000/api/posts')
        .then(response => response.json())
    ...
    .then(miembrosPost.map((id_member) => {
      console.log(id_member[0])
      // Añadir los miembros enlazados al post en members_posts
        fetch('http://localhost:8000/api/miembros_posts', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({id_post: posts[0][0], id_member: id_member[0]}),
        })
  ...
  const updatePost = (id, updatedPost) => {
    fetch(`http://localhost:8000/api/posts/${id}`, {
      ...
      .then(() => {
        // Borrar los miembros de la publicaion
        fetch(`http://localhost:8000/api/miembros_posts/post/${id}`, {
          method: 'DELETE',
          })
      })
      .then(miembrosPost.map((id_member) => {
        // Añadir los miembros a la publicaion
        ...
        fetch('http://localhost:8000/api/miembros_posts', {
            ...
            body: JSON.stringify({id_post: id, id_member: id_member[0]}),
          })
    ...
    return(
      <div>
        <div className="filtros">
          ...
          <input type="text" placeholder="Introduce el título" className="input-search" value={busqueda} onChange={e => setBusqueda(e.target.value)}></input>
          <button className="btn add"  onClick={handleAddDialogOpen}>Añadir publicación</button>
        </div>
        <Dialog open={openDialog} onClose={handleDialogClose}>
          <DialogTitle className="d-title">{editMode ? 'Editar publicación' : 'Añadir publicación'}</DialogTitle>
          <DialogContent>
            <TextField margin="dense" name="title" label="Título" type="text" fullWidth variant="outlined" value={newPost.title || ''} onChange={handleChange} />
            <TextField margin="dense" name="content" label="Descripción" type="text" fullWidth multiline rows={4} variant="outlined" value={newPost.content || ''} onChange={handleChange} />
            <TextField margin="dense" name="url" label="URL del vídeo" type="text" fullWidth multiline rows={4} variant="outlined" value={newPost.url || ''} onChange={handleChange} />
            <FormControl component="fieldset" variant="standard" margin="dense">
              <FormLabel component="legend">Miembros en la publicación:</FormLabel>
              <FormGroup>
              {miembros.map((item) => (
                <FormControlLabel
                  key={item[0]}
                  control={
                    <Switch
                      ...
                      name={item[1]}
                      checked={miembrosPost.some(element => element[0] === item[0])}
                    />
                  }
                  label={item[1]}
                />
          ...
          <DialogActions>
            ...
            <button className="btn confirmar dialog" onClick={() => { (newPost.title && newPost.content) ? savePost(newPost) : handleAlertOpen() }}>{editMode ? 'Guardar' : 'Añadir'}</button>
          </DialogActions>
        </Dialog>
        ...
        <Dialog open={openConfirm} onClose={handleConfirmClose}>
          <DialogTitle>Publicación {eliminar ? 'eliminada' : (editMode ? 'editada' : 'añadida')}</DialogTitle>
          <DialogActions>
            ...
            {postsFiltrados.map((post, key) => {
                return (
                    <tr key={key}>
                        ...
                        <td className="tabla-celda">{post[4].substring(11,16)} {post[4].substring(8,10)}{post[4].substring(4,8)}{post[4].substring(0,4)}</td>
                        <td className="tabla-celda">
                          <button className="btn editar" onClick={() =>  {handleEditDialogOpen(post[0], post)}}><AiFillEdit className="icon"/></button>
                        </td>
                        <td className="tabla-celda">
                          <button className="btn eliminar" onClick={() => {handleWarningOpen(post[0])}}><AiFillDelete className="icon"/></button>
                        </td>
                    </tr>
                )
            })}
